                                 ...
                                 kershaw_algorithm_data_t* kershaw_data)
{
    ...
    for (k = 2; k <= grid_size.nz - 1; k++)
    {
        ...
        r[nn] = source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        r[nn] = - 2*b1*wb_switch[wb_type]*boundary_funcs.boundary_west(Y[i][j][k], Z[i][j][k])
                - 2*b2*sb_switch[sb_type]*boundary_funcs.boundary_south(X[i][j][k], Z[i][j][k])
                - 2*b3*bb_switch[bb_type]*boundary_funcs.boundary_bottom(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
                - (A[nn][4]*xo[nn] + b1*xo[nn+1] + b2*xo[nn+grid_size.nx] +
                   b3*xo[nn+grid_size.nx*grid_size.ny]);
    }
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        r[nn] = - 2*b2*sb_switch[sb_type]*boundary_funcs.boundary_south(X[i][j][k], Z[i][j][k])
                - 2*b3*bb_switch[bb_type]*boundary_funcs.boundary_bottom(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        r[nn] = - 2*b1*eb_switch[eb_type]*boundary_funcs.boundary_east(Y[i][j][k], Z[i][j][k])
                - 2*b2*sb_switch[sb_type]*boundary_funcs.boundary_south(X[i][j][k], Z[i][j][k])
                - 2*b3*bb_switch[bb_type]*boundary_funcs.boundary_bottom(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        r[nn] = - 2*b3*bb_switch[bb_type]*boundary_funcs.boundary_bottom(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        r[nn] = - 2*b1*wb_switch[wb_type]*boundary_funcs.boundary_west(Y[i][j][k], Z[i][j][k])
                - 2*b2*nb_switch[nb_type]*boundary_funcs.boundary_north(X[i][j][k], Z[i][j][k])
                - 2*b3*bb_switch[bb_type]*boundary_funcs.boundary_bottom(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
    ...
    for (k = 1; k <= 1; k++)
    {
        ...
        r[nn] = - 2*b2*nb_switch[nb_type]*boundary_funcs.boundary_north(X[i][j][k], Z[i][j][k])
                - 2*b3*bb_switch[bb_type]*boundary_funcs.boundary_bottom(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
                - (b2*xo[nn-grid_size.nx] + b1*xo[nn-1] + A[nn][4]*xo[nn] + b1*xo[nn+1] +
                   b3*xo[nn+grid_size.nx*grid_size.ny]);
    ...
    for (k = grid_size.nz; k <= grid_size.nz; k++)
    {
        ...
        r[nn] = - 2*b1*wb_switch[wb_type]*boundary_funcs.boundary_west(Y[i][j][k], Z[i][j][k])
                - 2*b2*sb_switch[sb_type]*boundary_funcs.boundary_south(X[i][j][k], Z[i][j][k])
                - 2*b3*tb_switch[tb_type]*boundary_funcs.boundary_top(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
                - (b3*xo[nn-grid_size.nx*grid_size.ny] + A[nn][4]*xo[nn] + b1*xo[nn+1] +
    ...
    for (k = grid_size.nz; k <= grid_size.nz; k++)
    {
        ...
        r[nn] = - 2*b3*tb_switch[tb_type]*boundary_funcs.boundary_top(X[i][j][k], Y[i][j][k])
                + source(X[i][j][k], Y[i][j][k], Z[i][j][k]) + K*xo[nn]
                - (b3*xo[nn-grid_size.nx*grid_size.ny] + b2*xo[nn-grid_size.nx] + b1*xo[nn-1] +
                   A[nn][4]*xo[nn] + b1*xo[nn+1] + b2*xo[nn+grid_size.nx]);
    }
                   ...
                   double time_spent)
{
    ...
    //printf("error: %E\n", kershaw_data->epsilon);
    printf("Poisson3D iterations: %d\n", kershaw_data->iterations);
    printf("Poisson3D running time: %f\n", time_spent);
}
