                               ...
                               double ***T)
{
    ...
    if (file != NULL)
    {
        fprintf(file, "Potential along X at:\n");
                       ...
                       double ***ptr)
{
    ...
    if (file != NULL)
    {
        ...
        fprintf(file,"potential 1 %i float\n", nt);
        for (k = 1; k <= grid_size.nz ; k++)
        for (j = 1; j <= grid_size.ny ; j++)
        for (i = 1; i <= grid_size.nx; i++)
            fprintf(file,"%f ",ptr[i][j][k]);
                 ...
                 double ***T)
{
    ...
    char fnamepot[100];
    char fnamegrid[100];
    sprintf(fnamepot, "%s%s", fdir, "potential.txt");
    sprintf(fnamegrid, "%s%s", fdir, "GridX.txt");
    //printf("%s", fname);
    export_temperature_field(fnamepot, "Potential profile", grid_size, T);
    export_temperature_field(fnamegrid, "X grid coordinates", grid_size, grid_coordinates->X);

    /* disable the other output formats by Wenbin
    ...
    //export_T_data_alongz_atxy("Phi_alongz_atxy.gnumeric", grid_size, grid_coordinates,
    //                         ((grid_size.nx - 1)/2)+1, ((grid_size.ny - 1)/2)+1, T);
    //export_T_data_alongy_atxz("Phi_alongy_atxz.gnumeric", grid_size, grid_coordinates,
    //                         ((grid_size.nx - 1)/2)+1, ((grid_size.nz - 1)/2)+1, T);
    //export_T_data_alongx_atyz("Phi_alongx_atyz.gnumeric", grid_size, grid_coordinates,
